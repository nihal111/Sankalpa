...
describe('App create and reorder', () => {
  ...
  it('quick-add creates task in inbox (list_id = null)', async () => {
    ...
    setupMockApi({
      ...
      tasksCreate: vi.fn().mockResolvedValue({ id: 'new', list_id: null, title: '', status: 'PENDING', created_timestamp: 0, completed_timestamp: null, due_date: null, sort_key: 1, created_at: 0, updated_at: 0 }),
      tasksGetInbox: vi.fn().mockResolvedValue([{ id: 'new', list_id: null, title: '', status: 'PENDING', created_timestamp: 0, completed_timestamp: null, due_date: null, sort_key: 1, created_at: 0, updated_at: 0 }]),
    });
    render(<App />);
    await waitFor(() => expect(window.api.listsGetAll).toHaveBeenCalled());
  ...
  it('Cmd+N from tasks pane moves selection to new task', async () => {
    const newTask = { id: 'new', list_id: '1', title: '', status: 'PENDING', created_timestamp: 0, completed_timestamp: null, due_date: null, sort_key: 3, created_at: 0, updated_at: 0 };
