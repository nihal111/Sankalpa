...
export function seed(db: Database): void {
  ...
  const tutorialTasks = [
    ...
    'Hit E to edit a list name (when in lists pane)',
    'Hit D to set the due date of a task',
    'Hit ⌥D to set the duration of a task',
    'Hit N to edit notes for a task',
    'Use Tab and ⇧Tab to indent/outdent tasks (nesting)',
    'Hit C to collapse/expand a parent task',
    'Hold ⇧ and use ↑↓ arrow keys to create a contiguous selection',
    'Hold ⌃ and use ↑↓ + ↵ to create a discontiguous selection',
    'Create a selection and use M to move tasks to another list',
    'Hold ⌥ and use ↑↓ arrow keys to reorder a task',
    'Hit ⌃D to duplicate a task (deep copies collapsed subtrees)',
    'Hit ⌘C to copy task(s) as markdown',
    'Hit ⌘Z to undo, ⌘⇧Z to redo',
    'Hit ⌘K to open the command palette',
    'Hit ⌘⇧F to search across all tasks',
    'Hit ⌘, to open settings',
